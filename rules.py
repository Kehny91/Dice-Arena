...
class Deck:
    ...
    faces = [[("Attack2",3),("Heal1",1),("Sweep1",1),("Fireball1",1),("Armor2",2)],
            ...
            [("DemonicAxe",1), ("Mummyfy",1), ("Revive",1)],
            [("Tank",1), ("Vampire",1), ("King",1), ("Paladin",1), ("Lich",1), ("Barbarian",1), ("Thief",1),("Judge",1)]]
    
    allSpellsAndClass = [name for (name,mult) in faces[0]] + [name for (name,mult) in faces[1]] + [name for (name,mult) in faces[2]] + [name for (name,mult) in faces[3]] + [name for (name,mult) in faces[4]]
    nbOfDifferentDices1123CF = len(faces[0])*len(faces[0])*len(faces[1])*len(faces[2])*len(faces[4])
    ...
    def _init(cls):
        ...
        Deck._level5FacesWithMult = getListWithMultiplicity(Deck.faces[4])
        ...
        Deck._level5Faces = [name for (name,mult) in Deck.faces[4]]
        Deck._inited = True
    ...
    def getFacesWithMult(cls, tier):
        if not Deck._inited:
            Deck._init()
        assert tier>0 and tier<=5, f"No faces with tier {tier}"
        if tier == 1:
            ...
            return Deck._level5FacesWithMult
